...
namespace Day3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var cut in input)
            {
                var m = regex.Match(cut);
                var left = int.Parse(m.Groups["left"].Value);
                var top = int.Parse(m.Groups["top"].Value);
            ...
            foreach (var cut in input)
            {
                ...
                for (var x = left; x < left + width; ++x)
                {
                    for (var y = top; y < top + height; ++y)
                    {
                        good = good && (cloth[x, y] == 1);
                    }
                }

                if (good)
                {
                    IO.WriteLine(id.ToString());
                }
            }
        }
    }
