...
namespace _2019.Day17
{
    class Program
    {
        static Dictionary<Tuple<long, long>, char> board = new Dictionary<Tuple<long, long>, char>();
        ...
        static void Main(string[] args)
        {
            ...
            var total = 0;
            foreach ((var x, var y) in FindIntersections(arr))
            {
                total += x * y;
            }
            IO.WriteLine($"total coord: {total}");
        }
...
".Replace("\r", "");
        ...
        private static IEnumerable<(int, int)> FindIntersections(char[,] arr)
        {
            for (var y = 1; y < maxY; ++y)
            {
                for (var x = 1; x < maxX; ++x)
                {
                    if (arr[y, x] == '#' && arr[y - 1, x] == '#' && arr[y + 1, x] == '#' && arr[y, x - 1] == '#' && arr[y, x + 1] == '#')
                    {
                        yield return (x, y);
                    }
                }
        ...
        private static char[,] ConvertFromDictionary(Dictionary<Tuple<long, long>, char> dict)
        {
            maxX = dict.Keys.Max(k => k.Item1);
            maxY = dict.Keys.Max(k => k.Item2);
            var ret = new char[maxY + 1, maxX + 1];
            for (var y = 0L; y < maxY + 1; ++y)
            {
                for (var x = 0L; x < maxX + 1; ++x)
                {
                    ret[y, x] = dict[Tuple.Create(x, y)];
                }
            }
            return ret;
        }
        ...
        private static void HandleOutput(long item)
        {
            ...
            {
                board[Tuple.Create(curX++, curY)] = (char)item;
            }
        }
    }
}
